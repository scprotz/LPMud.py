...
def load_object(name, dont_reset):
    ...
    try:
        ...
        print("Could not read the file %s" % real_name, file=sys.stderr)

    ob = mud_object.get_empty_object(name)
    
    ob.name = name    # Shared string is no good here #
    obj_list.append(ob)
    
    otable.enter_object_hash(ob);    # add name to fast object lookup table #
 
    if (give_uid_to_object(ob) and not dont_reset):
        mud_object.reset_object(ob, 0)
    if (not (ob.O_DESTRUCTED) and interpret.function_exists("clean_up",ob)):
        ob.O_WILL_CLEAN_UP = True
...
# #if defined(_AIX) || defined(M_UNIX) || defined( __GNUC__)
#     struct dirent *de;
# #else
#     struct direct *de;
# #endif
...
# #if defined ( M_UNIX) || defined( __GNUC__)
